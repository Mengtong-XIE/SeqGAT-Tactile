...
class STBlock(nn.Module):
    def __init__(self, dim, heads=4, edge_dim_s=0, edge_dim_t=0, dropout=0.1):
        super().__init__()
        ...
        self.t_attn = TransformerConv(dim, dim//heads, heads=heads, edge_dim=(edge_dim_t if edge_dim_t>0 else None), beta=True, dropout=dropout)
        self.norm_s = nn.LayerNorm(dim); self.norm_t = nn.LayerNorm(dim)
        self.ff = MLP(dim, 4*dim, dim, dropout=dropout); self.norm_ff = nn.LayerNorm(dim)
    def forward(self, x, edge_index_s, edge_attr_s, edge_index_t, edge_attr_t):
        ...
        x = self.norm_s(x + xs)
        xt = self.t_attn(x, edge_index_t, edge_attr=edge_attr_t) if (edge_index_t is not None and edge_index_t.numel()>0) else torch.zeros_like(x)
        x = self.norm_t(x + xt)
        x = self.norm_ff(x + self.ff(x))
        return x

class SeqGAT(nn.Module):
    def __init__(self, in_dim, hidden_dim=128, num_layers=4, heads=4, edge_dim_s=0, edge_dim_t=0, time_emb_dim=32, dropout=0.1, readout="meanmax"):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([STBlock(hidden_dim, heads=heads, edge_dim_s=edge_dim_s, edge_dim_t=edge_dim_t, dropout=dropout) for _ in range(num_layers)])
    ...
    def forward(self, data):
        ...
        e_t = getattr(data,'edge_index_t', None); ea_t = getattr(data,'edge_attr_t', None)
        for blk in self.blocks: x = blk(x, e_s, ea_s, e_t, ea_t)
        # if hasattr(data,'t'):
        #     mx = data.t.max(); m = (data.t==mx); 
