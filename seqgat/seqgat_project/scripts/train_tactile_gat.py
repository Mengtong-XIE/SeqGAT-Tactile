...
def main(cfg):
    ...
    model = TactileGAT(in_dim=cfg["in_dim"], hidden_dim=cfg.get("hidden_dim",128), num_layers=cfg.get("num_layers",4), heads=cfg.get("heads",4),
                       edge_dim_s=cfg.get("edge_dim_s",0), time_emb_dim=cfg.get("time_emb_dim",32), dropout=cfg.get("dropout",0.1), readout=cfg.get("readout","meanmax")).to(device)
    ...
    for epoch in range(1, cfg.get("epochs",100)+1):
        ...
        if score < best:
            ...
            if cfg.get("save_ckpt",True):
                ...
                torch.save({"model":model.state_dict(), "cfg":cfg, "metrics":metrics}, os.path.join(cfg.get("out_dir","./outputs"), "tactile_gat_best.pth"))
...
if __name__ == "__main__":
    import yaml
    ap = argparse.ArgumentParser(); ap.add_argument("--config", type=str, default=os.path.join("configs","tactile_gat_k5_s.yaml"))
